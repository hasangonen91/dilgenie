...
- [ ] **A1ListeningData: clothes kategorisi** — `vocabulary/A1ListeningData.json` dosyasına `clothes` anahtarını ekle (aynı format, `food_drinks`'in hemen ardına gelmeli). `questions` dizisinde TAM 5 soru, konu: kıyafetler (shirt, hat, shoes, dress, coat vb.). Sıra bozma, diğer kategorilere dokunma.
